...
public class KamuiOnnna_AnnnaPrincess : CEntity_Effect
{
    public override List<ICardEffect> CardEffects(EffectTiming timing, CardSource card)
    {
        ...
        powerUpClass.SetUpICardEffect("夜刀神・長夜","", new List<Cost>(), new List<Func<Hashtable, bool>>() { CanUseCondition1 }, -1, false,card);
        ...
        bool CanUseCondition1(Hashtable hashtable)
        {
            if (card != null)
            {
                ...
                {
                    if (card.Owner.BondCards.Count((cardSource) => !cardSource.IsReverse && cardSource.cardColors.Contains(CardColor.Black)) >= 2)
                    {
                        return true;
                    }
                }
        ...
        changeCardColorsClass.SetUpICardEffect("白夜の心","", new List<Cost>(), new List<Func<Hashtable, bool>>() { CanUseCondition2 }, -1, false,card);
        changeCardColorsClass.SetUpCardColorChangeClass((cardSource, cardColors) => { cardColors.Add(CardColor.White); return cardColors; }, (cardSource) => cardSource == card);
