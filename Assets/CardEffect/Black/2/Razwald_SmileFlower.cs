...
using System;
using System.Linq;
public class Razwald_SmileFlower : CEntity_Effect
{
    public override List<ICardEffect> CardEffects(EffectTiming timing)
    {
        ...
        {
            activateClass[0].SetUpICardEffect("マークス隊のラズワルド", new List<Cost>(), new List<Func<Hashtable, bool>>() { CanUseCondition }, -1, false);
            activateClass[0].SetUpActivateClass((hashtable) => ActivateCoroutine());
            cardEffects.Add(activateClass[0]);

            if (ContinuousController.instance.language == Language.ENG)
            {
                activateClass[0].EffectName = "Xander's Retainer";
            }
            ...
            bool CanUseCondition(Hashtable hashtable)
            {
                if(IsExistOnField(hashtable))
                {
                    ...
                    {
                        if (card.Owner.FieldUnit.Count((unit) => unit.Character.UnitNames.Contains("マークス")) > 0)
                        {
                            return true;
                        }
                    }
