...
public class Marks_AnnyaKingSon : CEntity_Effect
{
    public override List<ICardEffect> CardEffects(EffectTiming timing)
    {
        ...
        {
            activateClass[0].SetUpICardEffect("神器 ジークフリート", new List<Cost>() { new ReverseCost(2, (cardSource) => true) }, null, -1, false);
            activateClass[0].SetUpActivateClass((hashtable) => ActivateCoroutine());
            cardEffects.Add(activateClass[0]);

            if (ContinuousController.instance.language == Language.ENG)
            {
                activateClass[0].EffectName = "Sacred Treasure, Siegfried";
            }
        ...
        strikeUpClass.SetUpICardEffect("暗夜の騎士道", new List<Cost>(), new List<Func<Hashtable, bool>>() { CanUseCondition }, -1, false);
