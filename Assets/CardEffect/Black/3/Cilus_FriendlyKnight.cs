...
public class Cilus_FriendlyKnight : CEntity_Effect
{
    public override List<ICardEffect> CardEffects(EffectTiming timing, CardSource card)
    {
        ...
        powerUpClass.SetUpICardEffect("騎士の誓い","", new List<Cost>(), new List<Func<Hashtable, bool>>() { CanUseCondition }, -1, false,card);
        ...
        bool CanUseCondition(Hashtable hashtable)
        {
            ...
            {
                if (card.Owner.FieldUnit.Count((_unit) => _unit.Character.UnitNames.Contains("カムイ(男)") || _unit.Character.UnitNames.Contains("カムイ(女)")) > 0)
                {
                    return true;
                }
            }
