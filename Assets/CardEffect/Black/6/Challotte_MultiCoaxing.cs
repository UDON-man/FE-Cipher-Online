...
public class Challotte_MultiCoaxing : CEntity_Effect
{
    public override List<ICardEffect> CardEffects(EffectTiming timing, CardSource card)
    {
        ...
        powerUpClass.SetUpICardEffect("私、か弱いので…", "", null, null, -1, false, card);
        ...
        bool CanPowerUpCondition(Unit unit)
        {
            if (card.UnitContainingThisCharacter() == unit)
            {
                ...
                {
                    if (card.Owner.FieldUnit.Count((_unit) => _unit.Character.sex.Contains(Sex.male) && _unit != card.UnitContainingThisCharacter()) >= 1)
                    {
                        return true;
                    }
                }
        ...
        powerUpClass1.SetUpICardEffect("ぶっとばすぞコラァ!", "", null, null, -1, false, card);
        powerUpClass1.SetUpPowerUpClass((unit, Power) => Power + 20, CanPowerUpCondition1, true);
        ...
        bool CanPowerUpCondition1(Unit unit)
        {
            if (card.UnitContainingThisCharacter() == unit)
            {
                ...
                {
                    if(card.Owner.FieldUnit.Count((_unit) => _unit.Character.sex.Contains(Sex.male) && _unit != card.UnitContainingThisCharacter()) == 0)
                    {
                        return true;
                    }
                }
