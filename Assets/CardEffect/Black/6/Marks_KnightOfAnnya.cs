...
using System;
using System.Linq;
public class Marks_KnightOfAnnya : CEntity_Effect
{
    public override List<ICardEffect> CardEffects(EffectTiming timing, CardSource card)
    {
        ...
        rangeUpClass.SetUpICardEffect("王族の血", "",null, new List<Func<Hashtable, bool>>() { CanUseCondition }, -1, false,card);
        rangeUpClass.SetUpRangeUpClass((unit, Range) => { Range.Add(1); Range.Add(2); return Range; }, (unit) => unit == card.UnitContainingThisCharacter());
        cardEffects.Add(rangeUpClass);

        PowerModifyClass powerUpClass = new PowerModifyClass();
        powerUpClass.SetUpICardEffect("王族の血","", null, new List<Func<Hashtable, bool>>() { CanUseCondition }, -1, false,card);
