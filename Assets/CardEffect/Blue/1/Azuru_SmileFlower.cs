...
using System;
using System.Linq;
public class Azuru_SmileFlower : CEntity_Effect
{
    public override List<ICardEffect> CardEffects(EffectTiming timing)
    {
        ...
        {
            activateClass[0].SetUpICardEffect("ナンパなアズール", new List<Cost>(), new List<Func<Hashtable, bool>>() { CanUseCondition }, -1, false);
            activateClass[0].SetUpActivateClass((hashtable) => ActivateCoroutine());
            cardEffects.Add(activateClass[0]);

            if (ContinuousController.instance.language == Language.ENG)
            {
                activateClass[0].EffectName = "Inigo the Flirt";
            ...
            bool CanUseCondition(Hashtable hashtable)
            {
                if (IsExistOnField(hashtable))
                {
                    ...
                    {
                        if (card.Owner.FieldUnit.Count((unit) => unit.Character.sex.Contains(Sex.female)) > 0)
                        {
                            return true;
                        }
                    }
