...
using System;
using System.Linq;
public class Ronkue_WestKingHeir : CEntity_Effect
{
    public override List<ICardEffect> CardEffects(EffectTiming timing, CardSource card)
    {
        ...
        powerUpClass.SetUpICardEffect("神剣闘技","", new List<Cost>(), new List<Func<Hashtable, bool>>() { CanUseCondition }, -1, false,card);
        ...
        bool CanUseCondition(Hashtable hashtable)
        {
            ...
            {
                if (card.Owner.FieldUnit.Count((_unit) => _unit != card.UnitContainingThisCharacter() && _unit.Weapons.Contains(Weapon.Sword)) >= 2)
                {
                    return true;
                }
            }
