...
public class Viorle_BrilliantFantomArcher : CEntity_Effect
{
    public override List<ICardEffect> CardEffects(EffectTiming timing)
    {
        ...
        if (timing == EffectTiming.OnDeclaration)
        {
            ...
            {
                if (card.Owner.FieldUnit.Count((unit) => unit.Character.UnitNames.Contains("弓弦エレオノーラ")) > 0)
                {
                    return true;
                }
        ...
        bool CanUseCondition(Hashtable hashtable)
        {
            if (card.UnitContainingThisCharacter() != null)
            {
                ...
                {
                    if (card.Owner.GetFrontUnits().Count((unit) => unit.Character.UnitNames.Contains("弓弦エレオノーラ")) > 0)
                    {
                        return true;
                    }
                }
                ...
                {
                    if (card.Owner.GetBackUnits().Count((unit) => unit.Character.UnitNames.Contains("弓弦エレオノーラ")) > 0)
                    {
                        return true;
                    }
                }
