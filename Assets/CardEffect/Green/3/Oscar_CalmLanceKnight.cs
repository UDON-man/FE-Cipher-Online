...
public class Oscar_CalmLanceKnight : CEntity_Effect
{
    public override List<ICardEffect> CardEffects(EffectTiming timing, CardSource card)
    {
        ...
        powerUpClass.SetUpICardEffect("穏やかな長兄","", null, null, -1, false,card);
        ...
        bool CanPowerUpCondition(Unit unit)
        {
            if (card.UnitContainingThisCharacter() == unit)
            {
                ...
                {
                    if (card.Owner.SupportCards.Count((cardSource) => cardSource.UnitNames.Contains("ボーレ") || cardSource.UnitNames.Contains("ヨファ")) > 0)
                    {
                        return true;
                    }
                }
