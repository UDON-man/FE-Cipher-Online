...
public class Nefenie_StrongestVillageGirl : CEntity_Effect
{
    public override List<ICardEffect> CardEffects(EffectTiming timing)
    {
        ...
        {
            activateClass[0].SetUpICardEffect("民兵の戦い方", new List<Cost>(), new List<Func<Hashtable, bool>>() { CanUseCondition }, -1, false);
            activateClass[0].SetUpActivateClass((hashtable) => ActivateCoroutine());
            cardEffects.Add(activateClass[0]);

            if (ContinuousController.instance.language == Language.ENG)
            {
                activateClass[0].EffectName = "Militia Woman";
            ...
            bool CanUseCondition(Hashtable hashtable)
            {
                if (IsExistOnField(hashtable))
                {
                    ...
                    {
                        if (card.Owner.FieldUnit.Count((unit) => card.Owner.GetFrontUnits().Contains(unit) && unit != card.UnitContainingThisCharacter()) >= 2)
                        {
                            return true;
                        }
                    }
