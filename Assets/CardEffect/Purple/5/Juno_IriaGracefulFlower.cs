...
public class Juno_IriaGracefulFlower : CEntity_Effect
{
    public override List<ICardEffect> CardEffects(EffectTiming timing, CardSource card)
    {
        ...
        bool CanPowerUpCondition(Unit unit)
        {
            if (card.UnitContainingThisCharacter() == unit)
            {
                ...
                {
                    if (card.Owner.SupportCards.Count((cardSource) => cardSource.UnitNames.Contains("ティト") || cardSource.UnitNames.Contains("シャニー")) > 0)
                    {
                        return true;
                    }
                }
        ...
        if (timing == EffectTiming.OnAttackAnyone)
        {
            SelectAllyCost selectAllyCost = new SelectAllyCost(
                SelectPlayer: card.Owner,
                    CanTargetCondition: (unit) => unit.Character.Owner == card.Owner && !unit.IsTapped && (unit.Character.UnitNames.Contains("ティト") || unit.Character.UnitNames.Contains("シャニー")),
            ...
            {
                if (PreSelectedUnits.Count((unit) => unit.Character.UnitNames.Contains("ティト")) > 0)
                {
                    if (addUnit.Character.UnitNames.Contains("ティト"))
                    {
                        return false;
                    }
                }
            ...
            {
                if (PreSelectedUnits.Count((unit) => unit.Character.UnitNames.Contains("ティト") && !unit.IsTapped) == 1 && PreSelectedUnits.Count((unit) => unit.Character.UnitNames.Contains("シャニー") && !unit.IsTapped) == 1)
                {
                    return true;
                }
