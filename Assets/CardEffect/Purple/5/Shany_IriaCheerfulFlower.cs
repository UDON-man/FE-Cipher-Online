    ...
    public override List<ICardEffect> CardEffects(EffectTiming timing, CardSource card)
    {
        ...
        bool CanPowerUpCondition(Unit unit)
        {
            if (card.UnitContainingThisCharacter() == unit)
            {
                ...
                {
                    if (card.Owner.SupportCards.Count((cardSource) => cardSource.UnitNames.Contains("ユーノ") || cardSource.UnitNames.Contains("ティト")) > 0)
                    {
                        return true;
                    }
                }
        ...
        if (timing == EffectTiming.OnAttackAnyone)
        {
            SelectAllyCost selectAllyCost = new SelectAllyCost(
                SelectPlayer: card.Owner,
                    CanTargetCondition: (unit) => unit.Character.Owner == card.Owner && !unit.IsTapped && (unit.Character.UnitNames.Contains("ユーノ") || unit.Character.UnitNames.Contains("ティト")),
            ...
            {
                if (PreSelectedUnits.Count((unit) => unit.Character.UnitNames.Contains("ユーノ") && !unit.IsTapped) == 1 && PreSelectedUnits.Count((unit) => unit.Character.UnitNames.Contains("ティト") && !unit.IsTapped) == 1)
                {
                    return true;
                }
