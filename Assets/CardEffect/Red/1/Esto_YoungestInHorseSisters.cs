...
public class Esto_YoungestInHorseSisters : CEntity_Effect
{
    public override List<ICardEffect> CardEffects(EffectTiming timing)
    {
        ...
        powerUpClass.SetUpICardEffect("三姉妹の絆", null, null, -1, false);
        ...
        bool CanPowerUpCondition(Unit unit)
        {
            if (card.UnitContainingThisCharacter() == unit)
            {
                ...
                {
                    if (card.Owner.SupportCards.Count((cardSource) => cardSource.UnitNames.Contains("パオラ") || cardSource.UnitNames.Contains("カチュア")) > 0)
                    {
                        return true;
                    }
                }
        ...
        if (timing == EffectTiming.OnAttackAnyone)
        {
            ...
            {
                if (PreSelectedUnits.Count((unit) => unit.Character.UnitNames.Contains("パオラ")) > 0)
                {
                    if (addUnit.Character.UnitNames.Contains("パオラ"))
                    {
                        return false;
                    }
                }
            ...
            {
                if (PreSelectedUnits.Count((unit) => unit.Character.UnitNames.Contains("パオラ") && !unit.IsTapped) == 1 && PreSelectedUnits.Count((unit) => unit.Character.UnitNames.Contains("カチュア") && !unit.IsTapped) == 1)
                {
                    return true;
                }
