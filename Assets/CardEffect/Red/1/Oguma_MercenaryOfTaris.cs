...
public class Oguma_MercenaryOfTaris : CEntity_Effect
{
    public override List<ICardEffect> CardEffects(EffectTiming timing, CardSource card)
    {
        ...
        powerUpClass.SetUpICardEffect("傭兵隊長","", null, null, -1, false,card);
        ...
        bool CanPowerUpCondition(Unit unit)
        {
            if (card.UnitContainingThisCharacter() == unit)
            {
                ...
                {
                    if (card.Owner.FieldUnit.Count((_unit) => _unit != card.UnitContainingThisCharacter() && _unit.Character.cEntity_Base.PlayCost <= 2) >= 2)
                    {
                        return true;
                    }
                }
