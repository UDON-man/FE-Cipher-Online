    ...
    public override List<ICardEffect> CardEffects(EffectTiming timing)
    {
        ...
        bool CanPowerUpCondition(Unit unit)
        {
            if (card.UnitContainingThisCharacter() == unit)
            {
                ...
                {
                    if (card.Owner.SupportCards.Count((cardSource) => cardSource.UnitNames.Contains("カチュア") || cardSource.UnitNames.Contains("エスト")) > 0)
                    {
                        return true;
                    }
                }
        ...
        if (timing == EffectTiming.OnAttackAnyone)
        {
            ...
            {
                if (PreSelectedUnits.Count((unit) => unit.Character.UnitNames.Contains("カチュア") && !unit.IsTapped) == 1 && PreSelectedUnits.Count((unit) => unit.Character.UnitNames.Contains("エスト") && !unit.IsTapped) == 1)
                {
                    return true;
                }
