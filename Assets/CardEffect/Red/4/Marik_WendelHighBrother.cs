...
public class Marik_WendelHighBrother : CEntity_Effect
{
    public override List<ICardEffect> CardEffects(EffectTiming timing)
    {
        ...
        {
            activateClass[1].SetUpICardEffect("カダインの門徒", new List<Cost>(), new List<Func<Hashtable, bool>>() { CanUseCondition }, 1, false);
            activateClass[1].SetUpActivateClass((hashtable) => ActivateCoroutine());
            cardEffects.Add(activateClass[1]);

            if (ContinuousController.instance.language == Language.ENG)
            {
                activateClass[1].EffectName = "Strudent of Khadein";
            }
                            ...
                            if (Unit != null)
                            {
                                if (Unit.Character != null)
                                {
                                    if (Unit.Character.Owner == this.card.Owner)
                                    {
                                        ...
                                        {
                                            if (Unit.Weapons.Contains(Weapon.MagicBook)|| Unit.Weapons.Contains(Weapon.Rod))
                                            {
                                                return true;
                                            }
                                        }
