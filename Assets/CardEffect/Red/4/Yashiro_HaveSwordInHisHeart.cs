...
public class Yashiro_HaveSwordInHisHeart : CEntity_Effect
{
    public override List<ICardEffect> CardEffects(EffectTiming timing)
    {
        ...
        {
            activateClass[0].SetUpICardEffect("戯れなら余所でやれ", new List<Cost>() , new List<Func<Hashtable, bool>>() { CanUseCondition }, 1, false);
            activateClass[0].SetUpActivateClass((hashtable) => ActivateCoroutine());
            cardEffects.Add(activateClass[0]);

            if (ContinuousController.instance.language == Language.ENG)
            {
                activateClass[0].EffectName = "What s Joke...";
            }

            bool CanUseCondition(Hashtable hashtable)
            {
                if (card.Owner.FieldUnit.Count((unit) => unit.Character.UnitNames.Contains("ナバール")) > 0)
                {
                    if (hashtable != null)
                    {
                            ...
                            if (hashtable["Defender"] is Unit)
                            {
                                ...
                                if (unit != null)
                                {
                                    if(unit.Character != null)
                                    {
                                        ...
                                        {
                                            if (!unit.Character.UnitNames.Contains("ナバール"))
                                            {
                                                return true;
                                            }
                                        }
