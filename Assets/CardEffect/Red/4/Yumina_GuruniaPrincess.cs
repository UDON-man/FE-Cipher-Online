...
public class Yumina_GuruniaPrincess : CEntity_Effect
{
    public override List<ICardEffect> CardEffects(EffectTiming timing, CardSource card)
    {
        ...
        if (timing == EffectTiming.OnDeclaration)
        {
            ...
            IEnumerator ActivateCoroutine()
            {
                ...
                selectCardEffect.SetUp(
                    CanTargetCondition: (cardSource) => !cardSource.UnitNames.Contains("ユミナ"),
        ...
        powerUpClass.SetUpPowerUpClass((unit, Power) => Power + 10, (unit) => unit.Character.Owner == card.Owner && (unit == card.UnitContainingThisCharacter() || unit.Character.UnitNames.Contains("ユベロ")), true);
        powerUpClass.SetUpICardEffect("双子の絆","", null, new List<Func<Hashtable, bool>>() { CanUseCondition }, -1, false,card);
        cardEffects.Add(powerUpClass);

        bool CanUseCondition(Hashtable hashtable)
        {
            if(card.Owner.FieldUnit.Count((unit) => unit.Character.UnitNames.Contains("ユベロ")) > 0)
            {
                return true;
            }
