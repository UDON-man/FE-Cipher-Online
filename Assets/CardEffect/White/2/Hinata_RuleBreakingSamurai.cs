...
using System;
using System.Linq;
public class Hinata_RuleBreakingSamurai : CEntity_Effect
{
    public override List<ICardEffect> CardEffects(EffectTiming timing)
    {
        ...
        {
            activateClass[0].SetUpICardEffect("タクミ隊のヒナタ", new List<Cost>(), new List<Func<Hashtable, bool>>() { CanUseCondition }, -1, true);
            activateClass[0].SetUpActivateClass((hashtable) => ActivateCoroutine());
            cardEffects.Add(activateClass[0]);

            if (ContinuousController.instance.language == Language.ENG)
            {
                activateClass[0].EffectName = "Takumi's Retainer";
            }
            ...
            bool CanUseCondition(Hashtable hashtable)
            {
                if (IsExistOnField(hashtable))
                {
                    ...
                    {
                        if (card.Owner.FieldUnit.Count((unit) => unit.Character.UnitNames.Contains("タクミ")) > 0)
                        {
                            return true;
                        }
                    }
