...
using System;
using System.Linq;
public class Setsuna_BonyariArcher : CEntity_Effect
{
    public override List<ICardEffect> CardEffects(EffectTiming timing, CardSource card)
    {
        ...
        if (timing == EffectTiming.OnAttackAnyone)
        {
            ...
            activateClass.SetUpICardEffect("ヒノカ隊のセツナ","Hinoka's Retainer", new List<Cost>(), new List<Func<Hashtable, bool>>() { CanUseCondition }, -1, false,card);
            ...
            bool CanUseCondition(Hashtable hashtable)
            {
                if (IsExistOnField(hashtable,card))
                {
                    ...
                    {
                        if (card.Owner.FieldUnit.Count((unit) => unit.Character.UnitNames.Contains("ヒノカ")) > 0)
                        {
                            return true;
                        }
                    }
