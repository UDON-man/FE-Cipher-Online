...
using System.Linq;
using System;
public class Shinonome_NotCompletedTarent : CEntity_Effect
{
    public override List<ICardEffect> CardEffects(EffectTiming timing, CardSource card)
    {
        ...
        powerUpClass.SetUpICardEffect("国を超えた友情", "",null, null, -1, false,card);
        ...
        bool CanPowerUpCondition(Unit unit)
        {
            if (unit == GManager.instance.turnStateMachine.AttackingUnit || unit == GManager.instance.turnStateMachine.DefendingUnit)
            {
                ...
                {
                    if (card.Owner.SupportCards.Count((_cardSource) => _cardSource.UnitNames.Contains("ジークベルト")) > 0)
                    {
                        return true;
                    }
                }
