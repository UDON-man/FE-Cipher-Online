...
using System;
using System.Linq;
public class Yuzu_CraneWingSwordman : CEntity_Effect
{
    public override List<ICardEffect> CardEffects(EffectTiming timing)
    {
        ...
        {
            activateClass[0].SetUpICardEffect("暗夜の客将", new List<Cost>(), new List<Func<Hashtable, bool>>() { CanUseCondition }, -1, false);
            activateClass[0].SetUpActivateClass((hashtable) => ActivateCoroutine());
            cardEffects.Add(activateClass[0]);

            if (ContinuousController.instance.language == Language.ENG)
            {
                activateClass[0].EffectName = "General of Nohr";
            }
            ...
            bool CanUseCondition(Hashtable hashtable)
            {
                if (IsExistOnField(hashtable))
                {
                    ...
                    {
                        if (card.Owner.FieldUnit.Count((unit) => unit.Character.cardColors.Contains(CardColor.Black)) > 0)
                        {
                            return true;
                        }
                    }
