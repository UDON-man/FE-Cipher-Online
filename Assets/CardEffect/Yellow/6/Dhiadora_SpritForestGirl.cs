...
using System;
using System.Linq;
public class Dhiadora_SpritForestGirl : CEntity_Effect
{
    public override List<ICardEffect> CardEffects(EffectTiming timing, CardSource card)
    {
        ...
        if (timing == EffectTiming.OnEnterFieldAnyone)
        {
            ...
            activateClass.SetUpICardEffect("運命の扉", "Threashold of Fate", new List<Cost>() , new List<Func<Hashtable, bool>>() { CanUseCondition }, -1, true,card);
                    ...
                    if (hashtable != null)
                    {
                        if (hashtable.ContainsKey("Unit"))
                        {
                            if (hashtable["Unit"] is Unit)
                            {
                                ...
                                {
                                    if(card.Owner.TrashCards.Count((cardSource) => cardSource.CanSetBondThisCard) > 0)
                                    {
                                        return true;
                                    }
                                }
            ...
            IEnumerator ActivateCoroutine()
            {
                ...
                if(selected)
                {
                    ...
                    selectCardEffect.SetUp(
                        ...
                        Message: "Select a card in your bond area to discard.",
                        MaxCount: 1,
                        CanEndNotMax: false,
                        isShowOpponent: true,
