...
public class Finn_KnightInTraining : CEntity_Effect
{
    public override List<ICardEffect> CardEffects(EffectTiming timing, CardSource card)
    {
        ...
        powerUpClass.SetUpICardEffect("レンスターの忠臣","", null, null, -1, false,card);
            ...
            if (unit == card.UnitContainingThisCharacter())
            {
                if (GManager.instance.turnStateMachine.AttackingUnit != null && GManager.instance.turnStateMachine.DefendingUnit != null)
                {
                    if (GManager.instance.turnStateMachine.AttackingUnit == unit || GManager.instance.turnStateMachine.DefendingUnit == unit)
                    {
                        ...
                        {
                            if (card.Owner.SupportCards.Count((cardSource) => cardSource.UnitNames.Contains("キュアン") || cardSource.UnitNames.Contains("エスリン") || cardSource.UnitNames.Contains("リーフ")) > 0)
                            {
                                return true;
                            }
                        }
